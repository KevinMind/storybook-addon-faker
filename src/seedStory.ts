...
export function seedStory<Args>(getStoryObj: FunctionOrObj<StoryObj<Args>> = () => ({})): StoryObj<Args> {
  if (isChromatic()) {
    faker.seed(STORYBOOK_FAKER_SEED);
  }

  const story = typeof getStoryObj === 'function' ? getStoryObj(STORYBOOK_FAKER_SEED) : getStoryObj;
